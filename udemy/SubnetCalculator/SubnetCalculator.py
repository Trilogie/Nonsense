...
def subnet_calc():
    try:
        ...
        while True:
            ...
            if (len(ip_octets) == 4) and (1 <= int(ip_octets[0]) <= 223) and (int(ip_octets[0]) != 127) \
            and (int(ip_octets[0]) != 169 or int(ip_octets[1]) != 254) \
        ...
        for octet in ip_octets:
            binary_ip = bin(int(octet)).lstrip('0b')
            ip_octets_binary.append(binary_ip.zfill(8))
        ...
        while True:
            ...
            if generate == "y":
                ...
                print("Random generated IP: %s" % y_iaddr)
                print("\n")
                continue
